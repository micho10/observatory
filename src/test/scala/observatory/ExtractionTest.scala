...
class ExtractionTest extends FunSuite {
  ...
  test("Extract the stations information from the stations file") {
    ...
    assertResult(14)(stations.count())
    ...
    assertResult(1)(stations.filter((station: Station) => station.id == "35048").count())
  }
  ...
  test("Extract the temperature information from the temperatures file") {
    ...
    assertResult(30)(temperatures.count())
    assertResult(0)(temperatures.filter((reading: TempReading) => reading.id == "012345").count())
    assertResult(16)(temperatures.filter((reading: TempReading) => reading.id == "049999").count())
    assertResult(3)(temperatures.filter((reading: TempReading) => reading.id == "049999-00003").count())
    assertResult(3)(temperatures.filter((reading: TempReading) => reading.id == "00003").count())
  ...
  test("Calculate yearly average for every location") {
    ...
    assertResult(0.5555555555555556)(yearlyAverage.find(_._1 == Location(72.58,-38.46)).)
  }
